...
function addAssetsToCache(assets: Asset[]) {
    ...
    const newAssets = assets.filter(asset => !existingAssets.some(a => a.id === asset.id));
...
function addPoolsToCache(pools: Pool[]) {
    ...
    const newPools = pools.filter(pool => !existingPools.some(p => p.id === pool.id));
...
function addMarketSearchesToCache(marketSearches: MarketSearch[]) {
    ...
    const newMarketSearches = marketSearches.filter(ms => !existingMarketSearches.some(e => e.id === ms.id));
