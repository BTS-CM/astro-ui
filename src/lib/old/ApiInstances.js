...
function createApiProxy(name) {
  return new Proxy([], {
    get: (_, method) => (...args) => Apis[name].exec(method, [...args])
  });
}

const db = createApiProxy("_db");
const network = createApiProxy("_net");
const history = createApiProxy("_hist");
const crypto = createApiProxy("_crypt");
const orders = createApiProxy("_orders");

/**
 * Function creates a new ApiInstance
 * @returns {Apis}
 */
const newApis = () => ({
  connect: (
