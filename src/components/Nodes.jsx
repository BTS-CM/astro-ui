    ...
    return (
      <div style={{ ...style }} key={`acard-${index}`}>
        <Card className="ml-2 mr-2">
          <CardHeader className={`pb-0 pt-0 ${index === 0 ? "bg-green-100" : ""}`}>
            <CardTitle>
              <div className={`grid grid-cols-4 gap-2 items-center mt-0 pt-0`}>
                ...
                <div className="text-right flex items-center justify-end">
                  <Button className="mr-2" variant="none" onClick={() => {
      ...
      <div className="container mx-auto mt-5 mb-5">
        <div className="grid grid-cols-1 gap-3">
          <Card>
            ...
            <CardContent>
              {usr && usr.chain && nodes[usr.chain] && nodes[usr.chain].length ? (
                <List
                  height={250}
              ...
              <div>
                ...
                <Input
                  ...
                  onKeyPress={(event) => {
                    if (
                      event.key !== 'Enter' ||
                      !inputURL ||
                      nodes[usr.chain].findIndex((node) => node.url === inputURL) !== -1 ||
                      !/^wss?:\/\/[a-zA-Z0-9.-]+\/ws$/.test(inputURL)
                    ) {
                      return;
                    }

                    updateNodes(usr.chain, [...nodes[usr.chain], {url: inputURL}])
                  }}
                />
                ...
                <Button onClick={() => updateNodes(usr.chain, chains[usr.chain].nodeList)}>
                  {t("Nodes:reset")}
                </Button>
              </div>
            </CardContent>
