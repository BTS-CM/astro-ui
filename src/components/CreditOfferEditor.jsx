...
const CreditOfferEditorCollateralRow = memo(function CreditOfferEditorCollateralRow({ index, style, acceptableCollateral, assets, t, selectedAsset, setAcceptableCollateral }) {
  ...
  const _displayPrice =
    Number.isFinite(_priceNum) && Number.isInteger(_targetAsset.precision)
      ...
      : res.price;
  return (<div style={{ ...style, paddingLeft: "8px", paddingRight: "8px", paddingBottom: "8px", paddingTop: "4px", overflow: "hidden" }} key={`acard-${res.id}`}><Card className="rounded-xl border border-[hsl(var(--accent-1)/0.15)] bg-card/60 py-0 gap-0 h-full overflow-hidden justify-center"><div className="p-3 flex items-center gap-3"><span className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[hsl(var(--accent-1)/0.3)]"> <Coins className="h-3.5 w-3.5" strokeWidth={2.25} /></span><div className="flex-1 min-w-0"><div className="text-sm font-semibold truncate">#{index + 1}: {_targetAsset.symbol}</div><div className="text-xs text-muted-foreground">{t("CreditOfferEditor:price")} <span className="font-mono">{_displayPrice}</span> {_targetAsset.symbol} / {selectedAsset ?? ""}</div></div><Button type="button" variant="ghost" size="icon" className="h-8 w-8 shrink-0 text-muted-foreground hover:text-[hsl(var(--accent-danger-fg))] hover:bg-[hsl(var(--accent-danger)/0.1)]" onClick={(event) => { event.preventDefault(); setAcceptableCollateral(acceptableCollateral.filter((_, i) => i !== index)); }}><Trash2 className="h-4 w-4" /></Button></div></Card></div>);
});
