    ...
    const ChosenPoolRow = ({ index, style }) => {
        ...
        return (
            <div style={style} key={`poolNo${index}`}>
                <Card
                    onClick={() => {
                        setSelectedPools(selectedPools.filter((poolId) => poolId !== pool.id));
                    }}
                >
                    <CardHeader className="p-0">
                        <CardDescription>
                            ...
                            <CardHeader className="pt-2 pb-2">
                                <CardDescription>
                                    ...
                                    <div className="grid grid-cols-1 gap-1">
                                        <p>{t("CustomPoolOverview:swappableAssets")}: {_uniqueAssets.join(', ')}</p>
                                        <p>{t("CustomPoolOverview:poolShareAssets")}: {
                                            _pools.map(x => _pools.length > 4 ? x.id : `${x.share_asset_symbol} (${x.id})`).join(', ')
                                        }</p>
                                    </div>
                                </CardDescription>
                            </CardHeader>
                    ...
                    <div className="grid grid-cols-6 gap-3">
                        <div className="col-span-5 border rounded border-gray-300 p-3">
                            <List
                                height={200}
                                itemCount={
                                    trackers && trackers[_chain]
                                        ? trackers[_chain].length
                                        : 0
                                }
                                itemSize={100}
                                className="w-full"
                        ...
                        <Button onClick={() => setModalOpen(true)}>{t("CustomPoolOverview:createTracker")}</Button>
                    </div>
                </CardContent>
            </Card>
                ...
                <DialogContent className="sm:max-w-[720px] bg-white">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        <div className="border rounded border-gray-300 p-2">
                            <List
                                height={200}
                                itemCount={selectedPools.length}
                                itemSize={30}
                                className="w-full"
                            >
                                {ChosenPoolRow}
                        ...
                        </div>
                        {
                            trackers && newTrackerTitle && selectedPools.length
                                ? <Button
                                    onClick={() => {
                                        const existingTracker = trackers[_chain].find(tracker => tracker.name === newTrackerTitle);
                                        if (existingTracker) {
                                        ...
                                        const newTracker = {
                                            ...
                                            pools: selectedPools,
                                            chain: _chain,
                                            id: toHex(sha256(newTrackerTitle))
                                        };
